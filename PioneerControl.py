...
class PioneerControl:
    def __init__(self, ip_addr, pioneer_port):
        ...
        self.__current_pos: tuple[float, float, float] = (0.0, 0.0, 0.0) # последняя полученная позиция дрона
        ...
        self.__coords_system: str = "GLOBAL_COORDS" # GLOBAL_COORDS / SPEED / SPEED_FIXED / SPEED_POINT
        ...
        self.__command_thread = threading.Thread(target=self.__command_loop, daemon=True)
            ...
            if self.__coords_system == "GLOBAL_COORDS":
                if self.__command_pos is not None and self.__armed and self.__takeoffed:
                    if prev_cmd != self.__command_pos:
                        self.pioneer.go_to_local_point(x=self.__command_pos[0], y=self.__command_pos[1], z=self.__command_pos[2], yaw=self.__yaw)
                ...
                if self.__armed and self.__takeoffed:
                    if prev_cmd != (*self.__speed, self.__yaw_rate):
                        self.pioneer.set_manual_speed(vx=self.__speed[0], vy=self.__speed[1], vz=self.__speed[2], yaw_rate=self.__yaw_rate)
                        self.logger.info(f"Command sent {self.__speed}, yaw_rate: {self.__yaw_rate}")
                ...
                if self.__armed and self.__takeoffed:
                    if self.__target_yaw is not None and self.__is_yaw_reached():
                        ...
                        self.__yaw_rate = 0
                    if prev_cmd != (*self.__speed, self.__yaw_rate):
                        self.pioneer.set_manual_speed_body_fixed(vx=self.__speed[0], vy=self.__speed[1], vz=self.__speed[2], yaw_rate=self.__yaw_rate)
                        self.logger.info(f"Command sent {self.__speed}, yaw_rate: {self.__yaw_rate}")
                        prev_cmd = (*self.__speed, self.__yaw_rate)
            elif self.__coords_system == "SPEED_POINT":
                if self.__command_pos is not None:
                     self.__speed = self.__compute_speed(*self.__command_pos, wanted_speed=self.target_speed)
                if self.__armed and self.__takeoffed:
                    self.pioneer.set_manual_speed_body_fixed(vx=self.__speed[0], vy=self.__speed[1], vz=self.__speed[2], yaw_rate=self.__yaw_rate)
    ...
    def __is_point_reached(self, dead_zone: float=0.7) -> bool:
        ...
        if self.__coords_system in ("GLOBAL_COORDS", "SPEED_POINT"):
            if self.__current_pos:
                distance = math.sqrt((self.__current_pos[0] - self.__command_pos[0]) ** 2 + (self.__current_pos[1] - self.__command_pos[1]) ** 2 + (self.__current_pos[2] - self.__command_pos[2]) ** 2)
                return distance < dead_zone
            return False
        else:
            self.logger.warning("Can't check if point is reached in current coordinate system")
            return True
    ...
    def __compute_speed(self, target_x: float, target_y: float, target_z: float, wanted_speed: float) -> tuple[float, float, float]:
        ...
        if distance < 2: # Начало зоны замедления
            current_speed = max(1, wanted_speed * (distance / 2)) # Минимальная скорость 0.2 м/с
    ...
    def set_speed_fixed(self, vx, vy, vz, yaw_rate=None):
        ...
        if yaw_rate is not None:
            self.__yaw_rate = math.radians(yaw_rate)
        self.logger.info(f"<<FIXED>> Speed set: {self.__speed}, yaw rate: {self.__yaw_rate}")
    ...
    def set_yaw(self, yaw, yaw_rate=30):
        if self.__coords_system == "SPEED_FIXED":
            ...
            self.__yaw_rate = math.radians(yaw_rate if self.get_yaw(degrees=True) < yaw else -yaw_rate)
